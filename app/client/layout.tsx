...
export default function ClientLayout({
    ...
    if (checking) {
        return (
            <div className="flex min-h-screen items-center justify-center bg-zinc-50 text-sm text-zinc-500 dark:bg-black dark:text-zinc-400">
                Проверяю доступ к кабинету клиента...
            </div>
        );
    }
    ...
    return (
        <div className="min-h-screen bg-zinc-50 text-zinc-900 dark:bg-black dark:text-zinc-50">
            <div className="mx-auto flex min-h-screen max-w-5xl gap-6 px-4 py-8 sm:px-8">
                <aside className="w-56 shrink-0 space-y-4 border-r border-zinc-200 pr-4 dark:border-zinc-800">
                    <div>
                    ...
                    <nav className="flex flex-col gap-1">
                        <NavItem href="/client/menus" label="Мой рацион" />
                        <NavItem
                            href="/client/history"
                            label="История рационов (скоро)"
                        />
                        <NavItem
                            href="/client/journal"
                            label="Дневник питания (скоро)"
                        />
                    </nav>

                    <p className="mt-4 text-[11px] text-zinc-500 dark:text-zinc-500">
                        Это режим клиента. Для работы нутрициолога используйте
                        раздел <span className="font-medium">/nutritionist</span>.
                    </p>
