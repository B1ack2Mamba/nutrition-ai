...
function foodValueToTokens(v: FoodValue): string[] {
    if (!v) return [];
    if (Array.isArray(v)) {
        return v.map((x) => String(x).trim()).filter(Boolean).slice(0, 60);
    }
    if (typeof v === "string") {
        return parseFoodTextareaToArray(v).slice(0, 60);
    }
    return [];
}
...
function pickFoodSnapshot(row: unknown): FoodDbSnapshot {
    ...
    if ("allowed_products" in row || "banned_products" in row) {
        const allowedRaw = row.allowed_products;
        const bannedRaw = row.banned_products;

        const allowed: FoodValue =
        ...
        const banned: FoodValue =
            Array.isArray(bannedRaw)
...
export default function ClientDetailPage() {
    ...
    // ✅ Можно / Нельзя (db snapshot + inputs)
    ...
    useEffect(() => {
        ...
        const uniq = Array.from(new Map(storedMenus.map((m) => [m.id, m])).values());
        setMenus(uniq);
    }, []);
    ...
    const reloadFoodRules = useCallback(
        async (nutritionistId: string) => {
            ...
            if (error) {
                setFoodHint(
                    "Секция «Можно/Нельзя» не настроена (таблица client_food_rules или права/RLS).",
                );
            ...
            // заполняем inputs из БД (чтобы нутрициолог видел “что сейчас” и мог править)
    ...
    const saveFoodRules = async () => {
        ...
        try {
            ...
            const tryLegacy = async (): Promise<string | null> => {
                ...
                if (foodDb.id) {
                    const { error } = await supabase.from("client_food_rules").update(payload).eq("id", foodDb.id);
                    return error ? error.message : null;
                } else {
                    const { error } = await supabase.from("client_food_rules").insert(payload);
                    return error ? error.message : null;
                }
            };
            ...
            // порядок: если schema известна — сначала её, иначе сначала legacy (чаще у тебя именно так)
    ...
    return (
        <div className="space-y-6">
            ...
            {/* Назначение рациона по цели + Можно/Нельзя */}
                ...
                <div className="flex items-start justify-between gap-3">
                    <div>
                        ...
                        <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
                            Слева — цель и рацион (компактно), справа — продукты «можно/нельзя» (видит клиент).
                        </p>
                    </div>
                    ...
                    <div className="rounded-2xl border border-zinc-200 bg-zinc-50 p-4 text-sm dark:border-zinc-700 dark:bg-zinc-900">
                        <div className="flex items-start justify-between gap-3">
                            <div>
                                ...
                                {foodDb.updatedAt ? (
                                    <div className="mt-1 text-[11px] text-zinc-500">
                                        Сейчас назначено (из БД): {formatDate(foodDb.updatedAt)}
                                    </div>
                                ) : (
                        ...
                        {foodSavedMsg ? (
                            <div className="mt-2 text-[11px] text-green-600 dark:text-green-400">
                                {foodSavedMsg}
                            </div>
                        ) : null}
                        ...
                        {/* ✅ ВИДНО “что назначено” (не textarea!) */}
                    ...
                    <div className="space-y-2 text-sm">
                        ...
                        <div className="space-y-2">
                            {shownAssignments.map((a) => {
                                const isActive =
                                    a.status === "active" || (a.status == null && a.id === activeAssignment?.id);
                        ...
                        {weightPath ? (
                            <div className="rounded-xl border border-zinc-200 bg-zinc-50 p-3 dark:border-zinc-700 dark:bg-zinc-900">
                                <div className="mb-2 flex items-center justify-between text-xs text-zinc-500">
                                    ...
                                    <span>
                                        {Math.round(weightPath.minW * 10) / 10} —{" "}
                                        {Math.round(weightPath.maxW * 10) / 10} кг
                                    </span>
                                </div>
