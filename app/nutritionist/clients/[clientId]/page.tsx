...
export default function ClientDetailPage() {
  ...
  const reloadLabReports = useCallback(async () => {
    // ✅ показываем и клиентские (nutritionist_id is null), и старые “от специалиста” (если были)
  ...
  const openLabReport = useCallback(async (r: LabReport) => {
    ...
    try {
      // ✅ ВАЖНО: только file_path. Если сюда сунуть URL — получишь “Object not found”
      const { data, error } = await supabase.storage.from("lab_reports").createSignedUrl(r.file_path, 60 * 10);

      if (error || !data?.signedUrl) {
        // fallback: если вдруг bucket public и в БД есть file_url
        if (r.file_url) {
  ...
  const setAssignmentStatus = async (id: string, status: "active" | "archived") => {
    ...
    const { error } = await supabase.from("client_menu_assignments").update({ status }).eq("id", id);
