...
type AutofillOut = {
    ...
    timeMinutes?: number;
    ingredients?: Array<{ name: string; amount?: string; basis?: "raw" | "cooked" }>;
    instructions?: string;
    notes?: string;
    macros?: {
        calories?: number | null;
...
export default function NewDishPage() {
    ...
    const sumIngredientCalories = useMemo(() => {
        const vals = ingredients
            .map((x) => (typeof x.calories === "number" ? x.calories : 0))
    ...
    const updateIngredient = (
        id: string,
        field: "name" | "amount" | "calories" | "basis",
        value: string,
    ) => {
        setIngredients((prev) =>
            prev.map((ing) =>
    ...
    const addIngredientRow = () => setIngredients((prev) => [...prev, makeIngredient()]);
    const removeIngredient = (id: string) => setIngredients((prev) => prev.filter((ing) => ing.id !== id));

    // === AI: автозаполнение блюда ===
    const handleAiAutofill = async () => {
        ...
        try {
            ...
            if (typeof out.title === "string" && out.title.trim()) setTitle(out.title.trim());
            if (out.category) setCategory(out.category);
            if (typeof out.timeMinutes === "number" && Number.isFinite(out.timeMinutes)) setTimeMinutes(out.timeMinutes);
    ...
    const onSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (!res.ok) {
                alert(`Не удалось сохранить: ${res.error}`);
                return;
            }
        ...
        <div className="flex flex-col gap-4">
            ...
            >
                ...
                <section className="space-y-2">
                    ...
                    {sumIngredientCalories != null && (
                        <p className="text-xs text-zinc-500">
                            Сумма ккал по ингредиентам (если ты заполнял ккал в строках): <b>{sumIngredientCalories}</b>
                        </p>
                    )}
                    ...
                    <div className="space-y-2">
                        {ingredients.map((ing, index) => (
                            ...
                            >
                                ...
                                <div className="grid gap-2 sm:grid-cols-[2fr_1.5fr_1fr_1fr]">
                                    <input
                                    ...
                                    <select
                                        value={ing.basis ?? ""}
                                        onChange={(e) => updateIngredient(ing.id, "basis", e.target.value)}
                                        className="rounded-lg border border-zinc-300 bg-transparent px-2 py-1.5 text-xs outline-none focus:border-zinc-900 dark:border-zinc-700 dark:focus:border-zinc-200"
                                        title="Вес указан как сырой/готовый"
                                    >
