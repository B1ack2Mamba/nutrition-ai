    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                if (!alive) return;
                setError(e?.message ?? "Не удалось загрузить блюда");
                setDishes([]);
            } finally {
        ...
        <div className="flex flex-col gap-4">
            <header className="flex items-center justify-between gap-4">
                ...
                <Link
                    ...
                    className="inline-flex items-center justify-center rounded-full bg-emerald-600 px-4 py-2 text-sm font-medium text-white hover:bg-emerald-700 dark:bg-zinc-100 dark:text-black dark:hover:bg-zinc-200"
                >
                    + Добавить блюдо
                </Link>
            </header>
            ...
            {error && (
                <div className="rounded-2xl border border-red-200 bg-red-50 p-3 text-sm text-red-700 dark:border-red-900 dark:bg-red-950/40 dark:text-red-200">
                    {error}
                </div>
            )}

            {loading ? (
                <div className="rounded-2xl border border-zinc-200 bg-white/70 p-6 text-sm text-zinc-600 dark:border-zinc-800 dark:bg-zinc-950/60 dark:text-zinc-300">
                    Загружаю блюда…
                </div>
            ) : dishes.length === 0 ? (
