...
async function ensureServiceWorkerControlsPage(timeoutMs = 6000): Promise<void> {
  ...
  // Some browsers may throw when subscribing if the page isn't yet controlled.
...
export default function NutritionistNotificationsPage() {
  ...
  const hasVapid = useMemo(() => !!process.env.NEXT_PUBLIC_VAPID_PUBLIC_KEY, []);
  ...
  const reload = async () => {
    ...
    if (prefRow) setPrefs(prefRow as Prefs);
      ...
      await db.from("notification_prefs").insert(def);
      setPrefs(def);
    }
  ...
  const enablePush = async () => {
    ...
    const { error } = await supabase
      ...
      .upsert(
        { user_id: user.id, kind: "webpush", endpoint, payload, updated_at: new Date().toISOString() },
    ...
    <div className="space-y-6">
      <header className="space-y-1">
        ...
        <p className="text-sm text-zinc-600 dark:text-zinc-400">
          Включите пуши, чтобы получать уведомления о дневнике и тренировках.
          На iPhone пуши работают только если сайт добавлен “На экран Домой”.
        </p>
      </header>
      ...
      <section className="rounded-2xl border border-zinc-200 bg-white p-5 text-sm shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
        ...
        <div className="mt-2 flex flex-wrap items-center gap-3">
          ...
          <a
            href="/install"
            className="text-xs text-zinc-600 underline hover:text-black dark:text-zinc-300 dark:hover:text-white"
          >
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          <label className="flex items-center justify-between gap-3 rounded-xl border border-zinc-200 px-3 py-2 dark:border-zinc-800">
            ...
            <input
              type="checkbox"
              checked={prefs?.enable_training ?? true}
              onChange={() => togglePref("enable_training")}
            />
          </label>
          ...
          <label className="flex items-center justify-between gap-3 rounded-xl border border-zinc-200 px-3 py-2 dark:border-zinc-800">
            ...
            <input
              type="checkbox"
              checked={prefs?.enable_diary ?? true}
              onChange={() => togglePref("enable_diary")}
            />
          </label>
        </div>
      </section>
